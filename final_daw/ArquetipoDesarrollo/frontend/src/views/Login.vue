    ...
    <div class="bg-surface-0 dark:bg-surface-900 p-8 md:p-12 shadow-sm rounded-2xl w-full max-w-sm flex flex-col gap-8">
      ...
      <div class="flex flex-col items-center gap-4">
        <div class="flex items-center">
          <div class="w-20 h-20 text-surface-900 dark:text-surface-0 shrink-0">
        ...
        <div class="flex flex-col items-center gap-2 w-full">
          <div class="text-surface-900 dark:text-surface-0 text-2xl font-semibold leading-tight text-center w-full">Iniciar sesión</div>
          <div class="text-center w-full">
            <span class="text-surface-700 dark:text-surface-200 leading-normal">¿No tienes cuenta?</span>
            <router-link to="/register" class="text-primary font-medium ml-1 cursor-pointer hover:underline">Regístrate</router-link>
          </div>
        </div>
      </div>
      ...
      <Form v-slot="$form" :resolver="resolver" :initialValues="initialValues" @submit="onFormSubmit" class="flex flex-col gap-6 w-full">
        ...
        <div class="flex flex-col gap-2 w-full">
          ...
          <Password name="password" placeholder="Tu contraseña" :toggleMask="true" :feedback="false" input-class="w-full!" fluid />
          <Message v-if="$form.password?.invalid" severity="error" variant="simple" size="small">
            {{ $form.password.error.message }}
          </Message>
        </div>

        <a class="text-primary font-medium cursor-pointer hover:underline text-sm self-end">¿Olvidaste tu contraseña?</a>
...
const onFormSubmit = async (event) => {
  ...
  try {
    ...
    apiError.value = true
    apiErrorMessage.value = getErrorMessage(error) || 'El correo o la contraseña no son correctos.'
  } finally {
    loading.value = false
  }
}
